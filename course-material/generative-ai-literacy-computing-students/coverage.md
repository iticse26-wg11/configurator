...
Generated 2026-08-24 from `config/config.yaml` (example: one-day workshop). Build v2 — rebuilt with the updated `build-course` skill; selection re-derived, placement done activities-first.
...
| EPR05 | S1 (revisited S4) | — presentation only (LA09 excluded by constraints) | — |
...
| Activity | Change | Why |
|----------|--------|-----|
| LA01 AI History Timeline | Condensed 45 → 15 min: 8 cards, no cross-group round, run after the history presentation | H has weight 1 → 15 min activity budget. Core mechanic — placing cards, then a reveal with deliberate surprises — survives at 15 min, so a shortened library activity is preferred over a generated one. |
...
**Sequencing check.** LA13 (S3) assumes MM01, MM02, EPR05 and CS02 are taught first: MM01 (S1/S2), MM02 (S2), CS02 (S2) and — moved from S4 in this build — EPR05 (S1). LA09 → LA07 ordering is moot (LA09 excluded). LA10 part 1 is the first block of S1; part 2 is the final homework.

**Selection check.** A global ILOs-per-minute greedy over the whole activity budget would take LA01 and LA13 at full length and leave no room for LA11 — dropping the only assessment-bearing activity for EPR10 and the main use of the policy attachment. The per-topic targets and the preference for assessment-bearing / attachment-using activities are what keep LA11 in; both are now explicit in the skill.
