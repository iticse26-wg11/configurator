...
**Sanity-check against the library — and say it plainly.** The activity library covers each ILO with exactly one activity, and most activities are 45–90 minutes. So for short courses many outcomes will be taught by presentation only. Estimate it now: count the activities relevant to their chosen ILOs (from `learning-activities/activities/*.md` frontmatter), sum their durations, and compare with the activity budget (`total_hours × 60 × (1 − presentation_share)`). Then tell them, in one or two sentences, e.g. *"With 6 hours and these four topics, there's about 3½ hours for activities and the matching library activities add up to 9 hours — so roughly half your outcomes will be covered by presentation rather than hands-on work. That's fine for an intro day; if you want more hands-on, the levers are fewer topics, more hours, or a lower presentation share."* Let them adjust before moving on. Also mention which activities need the instructor to *create* material beforehand (see the prep-burden list in `build-course` step 8) if any of those are likely to be selected.
